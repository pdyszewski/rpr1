...
function init() {
  ...
  let s = ''
  s += '<div id="main" style="position:relative; width:' + my.wd + 'px; min-height:' + my.ht + 'px;  margin:auto; display:block;   ">'
  ...
  s += '<div style="' + lblStyle + '">Lewo/Prawo:</div>'
  ...
  s += '<div style="' + lblStyle + '">Prędkość:</div>'
  ...
  //s += '<input  id="count" value=""/>'
  //s += getPlayHTML(36)
  s += '</div>'

  s += optPopHTML()

  //s += '<div id="copyrt" style="position: absolute; left: 10px; bottom: 2px; font: 10px Arial; color: blue; ">&copy; 2017 MathsIsFun.com  v' + this.version + '</div>'
  //s += '</div>'
...
//  g.stroke()
  ...
  // pegs
  //my.tiles = [];
  for (let row = 0; row < my.boardSize; row++) {
    //my.tiles[row] = [];
...
function docInsert(s) {
  let container = document.getElementById('galton-container-2')
  if (container) {
    container.innerHTML = s
  } else {
    console.error("Kontener 'galton-container-2' nie istnieje!")
  }
}
